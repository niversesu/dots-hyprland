...
> **Note:** this `sdata/dist-nix/home-manager/` config is the original WIP
> approach and is left here for reference. There is now a proper flake at the
> repo root (`flake.nix`, `nix/`) exposing `packages.default` and
> `homeManagerModules.default`, meant to be consumed as a flake input from
> your own home-manager config rather than run in place. See `FLAKE_USAGE.md`
> at the repo root.
